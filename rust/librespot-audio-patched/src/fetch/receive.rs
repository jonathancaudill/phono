...
async fn receive_data(
    ...
    let mut permit = shared.download_slots.acquire().await?;
        ...
        if code != StatusCode::PARTIAL_CONTENT {
            if code == StatusCode::TOO_MANY_REQUESTS {
                if let Some(duration) = HttpClient::get_retry_after(response.headers()) {
                    ...
                    permit = shared.download_slots.acquire().await?;
                    continue;
                }
            }
            ...
            if chunk_retries < MAX_CHUNK_RETRIES {
                chunk_retries += 1;
                let backoff = Duration::from_millis(250 * chunk_retries as u64);
                warn!("CDN chunk HTTP {code}, retry {chunk_retries}/{MAX_CHUNK_RETRIES} in {backoff:?}");
