...
use crate::{
    ...
    config::{OS, os_version, session_uses_non_platform_client_id},
...
impl SpClient {
    ...
    /// Drop a cached client-token so the next request mints with current identity.
    pub fn clear_client_token(&self) {
        self.lock(|inner| inner.client_token = None)
    }
    ...
    pub async fn client_token(&self) -> Result<String, Error> {
        ...
        // Keymaster OAuth on mobile must use session client_id + desktop_linux platform_data so
        // client-token matches login5 StoredCredential (see spotify-core auth.rs / PR #1403).
        let os = OS;
        let session_cid = self.session().client_id();
        let use_keymaster_desktop = session_uses_non_platform_client_id(&session_cid);
        let client_id = match os {
            "macos" | "windows" => session_cid.clone(),
            _ if use_keymaster_desktop => session_cid.clone(),
            os => SessionConfig::default_for_os(os).client_id,
        };
        client_data.client_id = client_id.clone();
        ...
        let platform_label = if use_keymaster_desktop && (os == "android" || os == "ios") {
            "linux"
        } else {
            os
        };
        ...
        info!(
            "Requesting client-token client_id={client_id} platform={platform_label}"
        );
    ...
    pub async fn request_with_options(
        ...
        loop {
            ...
            // Login5 first (psst path); OAuth fallback only when stored-credential refresh fails.
            let token = match self.session().login5().auth_token().await {
                Ok(token) => token,
                Err(e) => {
                    debug!("login5 auth_token failed ({e}); trying oauth_fallback");
                    match crate::oauth_fallback::oauth_fallback_token() {
                        Some(token) => token,
                        None => return Err(e),
                    }
                }
