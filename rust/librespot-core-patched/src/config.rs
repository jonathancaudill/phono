...
static OS_VERSION_OVERRIDE: OnceLock<String> = OnceLock::new();
static HTTP_PLATFORM_OVERRIDE: OnceLock<String> = OnceLock::new();

/// Override the OS version librespot sends to client-token/login5 (required on
/// Android when sysinfo returns an unusable value).
pub fn set_os_version_override(version: impl Into<String>) {
    let _ = OS_VERSION_OVERRIDE.set(version.into());
}

/// Override HTTP User-Agent platform label (e.g. `"linux"` for Keymaster OAuth on
/// Android so client-token/login5 share a desktop fingerprint with psst).
pub fn set_http_platform_override(platform: impl Into<String>) {
    let _ = HTTP_PLATFORM_OVERRIDE.set(platform.into());
}

/// Returns the HTTP User-Agent platform segment (`Android`, `Linux`, …).
pub fn http_platform_label() -> &'static str {
...
/// True when the session client id differs from the native platform default
/// (e.g. Keymaster OAuth on Android — client-token must align with login5).
pub fn session_uses_non_platform_client_id(session_client_id: &str) -> bool {
    session_client_id != SessionConfig::default_for_os(OS).client_id
}
...
pub fn os_version() -> String {
    if let Some(v) = OS_VERSION_OVERRIDE.get() {
        return v.clone();
    }
    let raw = sysinfo::System::os_version().unwrap_or_else(|| "0".into());
    if OS == "android" {
        // client-token/login5 reject invalid android_version (see comment above).
        if raw.parse::<u32>().ok().filter(|&n| n >= 21).is_some() {
            return raw;
        }
        return "30".to_string();
    }
